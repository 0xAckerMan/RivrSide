...
func (app *Application) HandleGetAllPackages(w http.ResponseWriter, r *http.Request) {
	...
	packagePlan := app.DB.Find(&packages)

	if packagePlan.RowsAffected == 0 {
		err := app.writeJSON(w, http.StatusOK, envelope{"package": "No package plan found at the moment"}, nil)
		if err != nil {
